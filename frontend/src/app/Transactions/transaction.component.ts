...
export class TransactionComponent { 
    ...
    headers: Headers;
  	...
  	ngOnInit()
  	{
  	  	this.http.get<Transaction[]>('http://localhost:8000/all-trans').subscribe(data => {
    ...
    addTrans(transCustomerId: number, transAmount: number): void {
        ...
        this.http.post('http://localhost:8000/post-trans', JSON.stringify(new Transaction(transCustomerId, transAmount)), httpOptions).subscribe(response => {
    ...
    updateTrans(transId: number, transAmount: number): void {
      ...
      this.http.post('http://localhost:8000/update-trans', JSON.stringify(new TransactionUpdate(transId, transAmount)), httpOptions).subscribe(response => {
          console.log(response);
          this.ngOnInit();
      });
    ...
    deleteTrans(checkedTrans: number): void{
      this.http.post('http://localhost:8000/delete-trans', JSON.stringify(new TransactionDelete(checkedTrans)), httpOptions).subscribe(response => {
          console.log(response);
          this.ngOnInit();
      });
    }
